...
{
    /* System exceptions */
    IRQn_NonMaskableInt     = -14,    // Non Maskable Interrupt, 1100
    IRQn_MemoryManagement   = -12,    // Cortex-M4 Memory Management, 1010
...
static inline uint32_t IS_IRQn_t(IRQn_t irq) { return (irq < 82 && irq >=0); }
static inline uint32_t IS_SYSTEM_HANDLER(IRQn_t irq) { return ( (irq == IRQn_NonMaskableInt) || \
                                                                (irq == IRQn_MemoryManagement) || \
                                                                (irq == IRQn_BusFault) || \
                                                                (irq == IRQn_UsageFault) || \
                                                                (irq == IRQn_SVCall) || \
                                                                (irq == IRQn_DebugMonitor) || \
                                                                (irq == IRQn_PendSV) || \
                                                                (irq == IRQn_SysTick) ); }
...
/* USART register base address definition */
